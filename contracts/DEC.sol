...
import "./lib/StructEnterprise.sol";
import "./IDEC.sol";

contract DEC is Ownable, IDEC {
  mapping(uint256 => EnterpriseLib.Enterprise) enterprises;
  ...
  function createEnterprise(
    ...
    EnterpriseLib.Enterprise storage enterprise = enterprises[countEnterprises];
  ...
  function investInEnterprise(
    uint256 enterpriseId
  ) external payable inCrowdfunding(enterpriseId) {
  ...
  function refoundInvest(
    uint256 enterpriseId,
    uint256 value
  ) external inCrowdfunding(enterpriseId) {
  ...
  function investissorTakePercent(
    uint256 enterpriseId,
    uint256 percent
  ) external isValidated(enterpriseId) {
    require(block.timestamp >= enterprises[enterpriseId].endAt);
  ...
  function founderRequestFounds(
    uint256 enterpriseId,
    uint256 request
  ) external isFounder(enterpriseId) isValidated(enterpriseId) {
  ...
  function investissorsAcceptRequest(
    uint256 enterpriseId
  ) external isValidated(enterpriseId) {
  ...
  function founderSendFounds(
    uint256 enterpriseId
  ) external payable isFounder(enterpriseId) isValidated(enterpriseId) {
    enterprises[enterpriseId].founds += msg.value;
